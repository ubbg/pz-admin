...
Not GitHub Issues: this checkout is a fork, and its issue list belongs to the upstream
project. Ausführbare Spezifikationen im Sinne von Spec-Driven Development liegen
dagegen unter `specs/` und sind versioniert — `.scratch/` ist der Arbeitsvorrat davor,
siehe `specs/README.md`.
...
- **Resolve**: append the answer under an `## Answer` heading, set `Status: resolved`,
  then append a context pointer (gist + link) to the map's Decisions-so-far in
  `map.md`.
